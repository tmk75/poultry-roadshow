window.APP_DATA = (function () {
  ...
  const KPIS = [
    ...
    { id: "water", label: { en: "Water & chemicals", zh: "水与化学品" }, value: "−19.6", unit: "%", target: { en: "−20%", zh: "−20%" }, trend: "down", good: true },
    { id: "decision", label: { en: "Decision speed", zh: "决策速度" }, value: "10.4", unit: "×", target: { en: "10× faster", zh: "10× 更快" }, trend: "up", good: true }
  ];
  ...
  const AGENTS = [
    ...
    { id: "safety2", name: { en: "Food Safety Sentinel", zh: "食品安全哨兵" }, role: { en: "Salmonella & Campylobacter", zh: "沙门氏菌与弯曲杆菌" }, color: "red", state: { en: "48h earlier warning on chilling water batch.", zh: "对预冷水批次提前 48 小时预警。" } }
  ];
  ...
  const FEED_EVENTS = [
    ...
    { en: "Energy agent requests setpoint increase — overridden.", zh: "能耗智能体请求提高设定值 —— 已被否决。" },
  ...
  const NEGOTIATION = [
    ...
    { color: "red", text: { en: "Safety: welfare score remains above threshold.", zh: "安全：福利评分仍高于阈值。" } }
  ];
  ...
  function stageReading(stage) {
    const val = rand(stage.min, stage.max, stage.unit === "°C" ? 1 : stage.unit === "A" ? 1 : 1);
    const bad = stage.id === "chilling" && val > 2.5;
    return { stage, value: val.toFixed(1), bad };
  }

  return { STAGES, KPIS, AGENTS, FEED_EVENTS, NEGOTIATION, stageReading };
})();
