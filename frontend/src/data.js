...
function fallbackSnapshot() {
  const stages = STAGES.map((stage) => {
    let value = rand(stage.min, stage.max, stage.unit === "%" ? 1 : 1);
    const bad = stage.id === "chilling" && value > 2.5;
    return { ...stage, value, bad };
  });
  return {
...
export { STAGES, KPIS, AGENTS, FEED_EVENTS, NEGOTIATION, fallbackSnapshot };
